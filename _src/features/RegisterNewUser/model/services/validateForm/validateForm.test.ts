...
describe('register validateForm', () => {
  ...
  test('with firstName error', async () => {
    expect(validateForm({ ...form, firstName: undefined })).toEqual([ValidationError.USER_DATA]);
  });

  test('with lastName error', async () => {
    expect(validateForm({ ...form, lastName: undefined })).toEqual([ValidationError.USER_DATA]);
  });
  ...
  test('with login error', async () => {
    expect(validateForm({ ...form, login: undefined })).toEqual([ValidationError.USER_DATA]);
  });

  test('with email error', async () => {
    expect(validateForm({ ...form, email: undefined })).toEqual([ValidationError.USER_DATA]);
  });
  ...
  test('with role error', async () => {
    expect(validateForm({ ...form, role: undefined })).toEqual([ValidationError.USER_DATA]);
  });
  ...
  test('with multiply errors', async () => {
    expect(
      validateForm({ ...form, confirmPassword: '123', password: '1111', login: undefined })
    ).toEqual([ValidationError.USER_DATA, ValidationError.PASSWORD_MATCH]);
  });

  test('with empty register error', async () => {
    expect(validateForm()).toEqual([ValidationError.NO_DATA]);
  });
});
