...
export const NewsPageFilter:FC<NewsPageFilterProps> = memo((props) => {
  ...
  const onChangeSort = useCallback((value: NewsSort) => {
  ...
  const onChangeType = useCallback((value: NewsType) => {
    ...
    <div className={classNames(cls.NewsPageFilter, {}, [className])}>
      <div className={cls.sortWrapper}>
        <div className={cls.sortSelects}>
          <Select<NewsSort> onChange={onChangeSort} value={sort} label={t('sort by')} options={sortOrderOption} />
      ...
      <Tabs<NewsType> className={cls.type} tabs={newsTypeTabs} value={type} onTabClick={onChangeType} />
    </div>
  );
});
