...
export type EditableNewsBlock = Partial<Omit<NewsBlock, 'id'>> & {
  localId: string;
  type: NewsBlockType;
  sequenceNumber: number;
};

export type EditableNewsBlockImage = Partial<Omit<NewsImageBlock, 'id' | 'src'>> &
  EditableNewsBlock & { image: string };

export interface EditableNewsBlockTextParagraph {
  localId: string;
  text: string;
}
export type EditableNewsBlockText = Partial<Omit<NewsTextBlock, 'id' | 'paragraphs'>> &
  EditableNewsBlock & { paragraphs: Array<EditableNewsBlockTextParagraph> };

export type EditableNewsBlockCode = Partial<Omit<NewsCodeBlock, 'id'>> &
  EditableNewsBlock & { code: string };
...
export function isEditableNewsBlockImage(
  block: EditableNewsBlock
): block is EditableNewsBlockImage {
